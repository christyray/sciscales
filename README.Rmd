...
You can install the development version of `sciscales` from [GitHub](https://github.com/) with:
...
`ggplot2`'s default formatting for large numbers is to print them plainly with no formatting, which can be hard to read or unsightly for very large numbers. 
...
```{r sciscales, fig.width=8, fig.asp=0.25, out.width="100%"}
library(ggplot2)
library(scales)
library(sciscales)
library(patchwork)
...
```{r zeros, fig.width=3, out.width="40%", fig.asp=0.75}
